...
func Paging(total int) *utils.Paging {
	...
	paging := utils.CreatePaging(1, 10, int64(total))
...
func Query(c *gin.Context) {
	...
	//_ = getQueriesByIds(realateSearchIds)

	// 查询query的全部DocId
	var query = model.Query{}
	db.MysqlDB.Where("query = ?", content).First(&query)
	docIdsList := utils.SplitDocIdsFromValue(query.DocIds)

	var docs []model.Docs
	// 根据DocId查询结果
	for _, v := range docIdsList {
		var doc model.Docs
		db.MysqlDB.Where("id = ?", v).First(&doc)
	...
	// 渲染HTML页面
	c.HTML(http.StatusOK, "index.tmpl", gin.H{"page": page, "results": docs[:page.Pagesize]})
