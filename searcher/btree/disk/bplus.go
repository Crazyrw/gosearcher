...
const (
	//0xdeadbeef 系统中 已分配但还未初始化的内存中用该数字来填充
...
func NewTree(filename string) (*Tree, error) {
	...
	fmt.Println("root: ", t.rootOff)
	fmt.Printf("freeBlocks: %v\n", t.freeBlocks)
	fmt.Println("blockSize: ", t.blockSize)
	fmt.Println("fileSize: ", t.fileSize)
	return t, nil
}
...
//func insertKeyValIntoNode(n *Node, key uint64, child OFFTYPE) (int, errs) {
//	idx := sort.Search(len(n.Keys), func(i int) bool {
//		return key <= n.Keys[i]
//	})
//	if idx < len(n.Keys) && n.Keys[idx] == key {
...
//func (t *Tree) Update(key uint64, val string) errs {
//	var (
//		node *Node
//		err  errs
//	)
...
//func (t *Tree) deleteKeyFromNode(off OFFTYPE, key uint64) errs {
//	if off == INVALID_OFFSET {
//		return nil
//	}
//	var (
...
//func (t *Tree) deleteKeyFromLeaf(key uint64) errs {
//	var (
//		leaf     *Node
//		prevLeaf *Node
//		nextLeaf *Node
...
//}
//
//func (t *Tree) Delete(key uint64) errs {
//	if t.rootOff == INVALID_OFFSET {
//		return fmt.Errorf("not found key:%d", key)
//	}
